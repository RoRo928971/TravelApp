...
- [x] presence（相手のカーソル・編集中表示）のリアルタイム連携
- [ ] 日（Day）の追加・並べ替え
- [ ] 行程の共有・PDF/印刷出力

### リアルタイム presence（実装済み）
Supabase Realtime の presence チャンネル（`presence:<tripId>`）で、各自のオンライン状態と
「いま何を編集中か」を track。相手の状態をヘッダー（オンライン / ○○が編集中…）と、
該当する予定カード・チェック項目の「編集中」枠に反映します（クラウドモードのみ）。
